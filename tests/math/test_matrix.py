...
class TestMatrix:
    def test_given_shape_when_creating_random_matrix_then_check_matrix_has_correct_shape(self, mock_weights_range):
        test_rows, test_cols = (3, 2)
        test_matrix = Matrix.random_matrix(
            rows=test_rows, cols=test_cols, low=mock_weights_range[0], high=mock_weights_range[1]
        )
    ...
    def test_given_shape_when_creating_random_column_then_check_matrix_has_correct_shape(self, mock_weights_range):
        test_rows, test_cols = (3, 1)
        test_matrix = Matrix.random_column(rows=test_rows, low=mock_weights_range[0], high=mock_weights_range[1])
    ...
    def test_given_2d_array_when_creating_matrix_then_check_matrix_has_correct_shape(self, mock_weights_range):
        test_rows, test_cols = (4, 5)
        test_array = np.random.uniform(
            low=mock_weights_range[0], high=mock_weights_range[1], size=(test_rows, test_cols)
        )
    ...
    def test_given_1d_array_when_creating_matrix_then_check_matrix_has_correct_shape(self, mock_weights_range):
        test_rows, test_cols = (4, 1)
        test_array = np.random.uniform(low=mock_weights_range[0], high=mock_weights_range[1], size=(test_rows))
        test_matrix = Matrix.from_array(matrix_array=test_array)
    ...
    def test_given_matrix_when_mapping_then_check_new_matrix_correctly_calculated(self, mock_activation):
        ...
        new_matrix = Matrix.map(test_matrix_1, mock_activation)
